...
{
    for(NSEntityDescription *entity in [CoreData coreData].managedObjectModel)
    {
        Class objectClass = NSClassFromString(entity.name);
        [self deleteObjectsForClass:objectClass];
    }
...
{
    NSArray *objects = [objectClass fetchObjectsForPredicate:nil sortedBy:nil];
...
- (void)objectsChanged:(NSNotification*)notification
{
    [CoreData save];
}
...
- (void)saveContext
{
    ...
    if (managedObjectContext != nil)
    {
        ...
        if ([managedObjectContext hasChanges] && ![managedObjectContext save:&error])
        {
            ...
            // abort() causes the application to generate a crash log and terminate. You should not use this function in a shipping application, although it may be useful during development.
            NSLog(@"Coredata schema mismatch error %@, %@", error, [error userInfo]);
            abort();
        }
    }
}
